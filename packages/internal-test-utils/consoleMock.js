...
export function createLogAssertion(
  ...
  return function assertConsoleLog(expectedMessages, options = {}) {
    if (__DEV__) {
      ...
      // TODOL Remove withoutStack option once all tests include component stacks.
      // This option will become unnecessary.
      const withoutStack = options.withoutStack;

      // withoutStack validation - can be removed when withoutStack is removed.
      ...
      for (let index = 0; index < observedLogs.length; index++) {
        ...
        let expectedMessage;
        // withoutStack per-message option - can be simplified when withoutStack is removed.
        ...
        if (
          ...
          // withoutStack: [message, {withoutStack: true}] form - can be removed when withoutStack is removed.
        ...
        if (typeof expectedMessage === 'string') {
          if (normalizedMessage === expectedMessage) {
            ...
            // withoutStack: partial matching for unexpected component stacks.
            // Can be removed when withoutStack is removed (tests will include full stacks).
            const remainder = normalizedMessage.slice(expectedMessage.length);
            if (isLikelyAComponentStack(remainder)) {
              matchesExpectedMessage = true;
            }
          } else if (
            isLikelyAComponentStack(expectedMessage) &&
            normalizedMessage.startsWith(expectedMessage)
          ) {
            // When the expected message includes a component stack but the actual
            // message has additional stacks (duplicate stacks), allow partial matching
            // so the "duplicate component stack" check can trigger.
            const remainder = normalizedMessage.slice(expectedMessage.length);
            if (isLikelyAComponentStack(remainder)) {
              matchesExpectedMessage = true;
            }
